...
function list({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  if (!deployments || (Array.isArray(deployments) && deployments.length <= 0)) {
    const match = yield now.findDeployment(app)
    if (match !== null && typeof match !== 'undefined') {
      deployments = Array.of(match)
    }
  }
  if (!deployments || (Array.isArray(deployments) && deployments.length <= 0)) {
    const aliases = yield now.listAliases()

    const item = aliases.find(e => e.uid === app || e.alias === app)
    if (item) {
      const match = yield now.findDeployment(item.deploymentId)
      if (match !== null && typeof match !== 'undefined') {
        deployments = Array.of(match)
      }
    }
  ...
  if (argv.all) {
    yield Promise.all(
      deployments.map(({ uid }, i) => __async(function*(){
        deployments[i].instances = yield now.listInstances(uid)
      }()))
    )
  }
