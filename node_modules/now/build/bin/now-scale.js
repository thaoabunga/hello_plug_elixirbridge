...
function run({ token, config: { currentTeam } }) {return __async(function*(){
  ...
  const deployments = yield scale.list()
  ...
  if ((match.state === 'FROZEN' || match.scale.current === 0) && min > 0) {
    ...
    if (match.scale.max < 1) {
      yield scale.setScale(match.uid, { min: 0, max: 1 })
    }
    yield scale.unfreeze(match)
  }

  const { min: newMin, max: newMax } = yield scale.setScale(match.uid, {
    min,
    max
  })
  ...
  log()
  yield info(scale, match.url)

  scale.close()
}())}

function list(scale) {return __async(function*(){
  let deployments
  try {
    const app = argv._[1]
    deployments = yield scale.list(app)
  } catch (err) {
    handleError(err)
    process.exit(1)
  }
