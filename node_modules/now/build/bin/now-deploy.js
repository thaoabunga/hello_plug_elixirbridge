...
function main() {return __async(function*(){
  let config = yield cfg.read({ token: argv.token })
  ...
  if (!token || shouldLogin) {
    try {
      token = yield login(apiUrl)
      config = yield cfg.read()
    } catch (err) {
  ...
  try {
    yield sync({ token, config })
  } catch (err) {
    return stopDeployment(`Unknown error: ${err}\n${err.stack}`)
  }
}())}

function sync({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  if (plan.id === 'oss') {
    if (isTTY) {
      ...
      try {
        ...
        proceed = yield promptBool(label, { trailing: eraseLines(1) })
      } catch (err) {
      ...
      if (!proceed) {
        ...
        yield now.remove(now.id, { hard: true })
        stopSpinner()
