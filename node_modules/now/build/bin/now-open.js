...
function open({ token, config: { currentTeam, user } }) {return __async(function*(){
  const now = new Now({ apiUrl, token, debug, currentTeam })

  let deployments
  try {
    deployments = yield now.list(app)
  } catch (err) {
    handleError(err)
    process.exit(1)
  }
  ...
  const [currentProjectDeployments] = yield getCurrentProjectDeployments(
    [...apps],
    pkg
  )
  ...
  const sorted = yield sortByCreation([...currentProjectDeployments])
...
function getCurrentProjectDeployments(apps, pkg) {return __async(function*(){
  return apps.filter(app => pkg.name === app[0]).map(app => app[1])
}())}

function sortByCreation(deps) {return __async(function*(){
