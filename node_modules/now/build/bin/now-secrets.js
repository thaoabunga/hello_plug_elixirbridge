...
function run({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  if (subcommand === 'rm' || subcommand === 'remove') {
    ...
    if (theSecret) {
      const yes = yield readConfirmation(theSecret)
      if (!yes) {
        error('User abort')
        return exit(0)
      }
    ...
    const secret = yield secrets.rm(args[0])
    const elapsed = ms(new Date() - start)
  ...
  if (subcommand === 'rename') {
    ...
    const secret = yield secrets.rename(args[0], args[1])
    const elapsed = ms(new Date() - start)
  ...
  if (subcommand === 'add' || subcommand === 'set') {
    ...
    const secret = yield secrets.add(name, value)
    const elapsed = ms(new Date() - start)
