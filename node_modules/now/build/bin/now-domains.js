...
function run({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  switch (subcommand) {
    ...
    case 'remove': {
      ...
      const _domains = yield domain.ls()
      ...
      try {
        const confirmation = (yield readConfirmation(
          domain,
          _domain,
          _domains
        )).toLowerCase()
    ...
    case 'set': {
      ...
      const { uid, code, created, verified } = yield domain.add(
        name,
        argv.force,
        argv.external
      )
    ...
    case 'buy': {
      yield require(resolve(__dirname, 'domains', 'buy.js'))({
        domains: domain,
        args,
        currentTeam,
        user
...
function readConfirmation(domain, _domain) {return __async(function*(){
