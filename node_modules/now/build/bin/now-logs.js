...
function printLogs({ token, config: { currentTeam } }) {return __async(function*(){
  let buf = []
  let init = false
  let lastLog

  if (!follow) {
    onLogs(yield fetchLogs({ token, currentTeam, since, until }))
    return
  }
...
function fetchLogs({ token, currentTeam, since, until } = {}) {return __async(function*(){
  const now = new Now({ apiUrl, token, debug, currentTeam })

  let logs
  try {
    logs = yield now.logs(deploymentIdOrURL, {
      types,
      limit,
      query,
      since,
