...
function run({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  if (subcommand === 'ls' || subcommand === 'list') {
    ...
    if (argv.crt || argv.key || argv.ca) {
      ...
      cert = yield certs.put(cn, crt, key, ca)
    } else {
      // Issue a standard certificate
      cert = yield certs.create(cn)
    }
    if (!cert) {
    ...
    const cert = yield getCertIdCn(certs, args[0], currentTeam, user)
    if (!cert) {
      return exit(1)
    }
    const yes = yield readConfirmation(
      cert,
    ...
    yield certs.put(cert.cn, crt, key, ca)
    const elapsed = ms(new Date() - start)
...
function getCertIdCn(certs, idOrCn, currentTeam, user) {return __async(function*(){
  const list = yield certs.ls()
