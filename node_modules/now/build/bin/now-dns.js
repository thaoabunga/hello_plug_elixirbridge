...
function run({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  if (subcommand === 'ls' || subcommand === 'list') {
    ...
    const res = yield domainRecords.ls(args[0])
    const text = []
    let count = 0
    ...
    const record = yield domainRecords.create(param.domain, param.data)
    const elapsed = ms(new Date() - start)
    ...
    const record = yield domainRecords.getRecord(args[0])
    if (!record) {
    ...
    yield domainRecords.delete(record.domain, record.id)
    const elapsed = ms(new Date() - start)
