...
function updateCurrentTeam({ cfg, newTeam } = {}) {return __async(function*(){
  delete newTeam.created
  delete newTeam.creator_id
  yield cfg.merge({ currentTeam: newTeam })
}())}

module.exports = function({ teams, args, token }) {return __async(function*(){
  let stopSpinner = wait('Fetching teams')
  const list = (yield teams.ls()).teams
  let { user, currentTeam } = yield cfg.read({ token })
