...
function remove({ token, config: { currentTeam } }) {return __async(function*(){
  const now = new Now({ apiUrl, token, debug, currentTeam })

  const deployments = yield now.list()
  ...
  const aliases = yield Promise.all(
  ...
  try {
    if (!skipConfirmation) {
      const confirmation = (yield readConfirmation(matches)).toLowerCase()
    ...
    yield Promise.all(matches.map(depl => now.remove(depl.uid, { hard })))
