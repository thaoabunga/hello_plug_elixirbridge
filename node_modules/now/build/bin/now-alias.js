...
if (argv.help) {
  ...
  Promise.resolve().then(() => __async(function*(){
    const config = yield cfg.read({ token: argv.token })

    let token
    try {
      token = config.token || ((yield login(apiUrl)))
    } catch (err) {
...
function run({ token, config: { currentTeam, user } }) {return __async(function*(){
  ...
  switch (subcommand) {
    ...
    case 'list': {
      if (args.length === 1) {
        const list = yield alias.listAliases()
      ...
      const list = yield alias.list()
      const urls = new Map(list.map(l => [l.uid, l.url]))
      const aliases = yield alias.ls()
    ...
    case 'remove': {
      ...
      try {
        const confirmation = yield confirmDeploymentRemoval(alias, _alias)
        if (!confirmation) {
          console.log('\n> Aborted')
          process.exit(0)
        }
    ...
    case 'set': {
      if (argv.rules) {
        yield updatePathAlias(alias, argv._[0], argv.rules, domains)
        break
      }
      if (args.length !== 2) {
...
function confirmDeploymentRemoval(alias, _alias) {return __async(function*(){
  const deploymentsList = yield alias.list()
...
function updatePathAlias(alias, aliasName, rules, domains) {return __async(function*(){
  const start = new Date()
  const res = yield alias.updatePathBasedroutes(
    String(aliasName),
    rules,
    domains
  )
