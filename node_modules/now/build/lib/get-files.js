...
const glob = function(pattern, options) {return __async(function*(){
...
const maybeRead = function(path, default_ = '') {return __async(function*(){
  try {
...
function npm(
  ...
  const npmIgnore = yield maybeRead(resolve(path, '.npmignore'), null)
  const gitIgnore = npmIgnore === null
    ? yield maybeRead(resolve(path, '.gitignore'))
    : null
  ...
  const files = yield explode(search, {
    accepts,
    limit,
    debug
  })
...
function docker(
  ...
  // Compile list of ignored patterns and files
  const dockerIgnore = yield maybeRead(resolve(path, '.dockerignore'), null)
  ...
  const files = yield explode(search, { accepts, limit, debug })
...
function explode(paths, { accepts, debug }) {return __async(function*(){
  const list = file => __async(function*(){
    let path = file
    let s
  ...
  return flatten(yield many(paths)).filter(v => v !== null)
}())}
