...
module.exports = function(now, url) {return __async(function*(){
  const match = yield now.findDeployment(url)
  ...
  const instances = yield retry(
    () => __async(function*(){
      const res = yield now.listInstances(match.uid)
