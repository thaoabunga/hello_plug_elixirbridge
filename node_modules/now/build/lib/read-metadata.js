...
function readMetaData(
  ...
  let isStatic = false
  let pkg = yield readJSON(path, 'package.json')
  let nowConfig = yield readJSON(path, 'now.json')
  const dockerfile = yield readDockerfile(path)
...
function readJSON(path, name) {return __async(function*(){
  try {
...
function readDockerfile(path, name = 'Dockerfile') {return __async(function*(){
  try {
    const contents = yield readFile(resolvePath(path, name), 'utf8')
