 ...
 * @return {Object}
 */
function read({ force = false, token, apiUrl } = {}) {return __async(function*(){
  let existing = {}
  try {
  ...
  if ((force && token) || (token && token !== existing.token)) {
    const user = yield getUser({ token, apiUrl })
    if (user) {
      return {
        token,
        user: {
 ...
 * @param {Object} data
 */
function merge(data) {return __async(function*(){
  const cfg = Object.assign({}, yield read(), data)
  save(cfg)
}())}

// Removes a key from the config and store the result
function remove(key) {return __async(function*(){
...
const removeFile = () =>__async(function*(){ return fs.remove(file)}())
