...
module.exports = class Alias extends Now {
  ls(deployment) {return __async(function*(){
    if (deployment) {
      const target = yield this.findDeployment(deployment)
  ...
  rm(_alias) {return __async(function*(){
    return this.retry(bail => __async(function*(){
      const res = yield this._fetch(`/now/aliases/${_alias.uid}`, {
        method: 'DELETE'
      })
  ...
  findDeployment(deployment) {return __async(function*(){
  ...
  updatePathBasedroutes(alias, rules, domains) {return __async(function*(){
    alias = yield this.maybeSetUpDomain(alias, domains)
    return this.upsertPathAlias(alias, rules)
  }.call(this))}

  upsertPathAlias(alias, rules) {return __async(function*(){
    return this.retry((bail, attempt) => __async(function*(){
      ...
      const res = yield this._fetch(`/now/aliases`, {
        method: 'POST',
      ...
      if (body.error) {
        ...
        if (code === 'cert_expired') {
          ...
          try {
            yield this.createCert(alias, { renew: true })
          } catch (err) {
            return bail(err)
          }
        }
  ...
  set(deployment, alias, domains, currentTeam, user) {return __async(function*(){
    const depl = yield this.findDeployment(deployment)
    if (!depl) {
      const err = new Error(
    ...
    const aliasDepl = (yield this.listAliases()).find(e => e.alias === alias)
    if (aliasDepl && aliasDepl.rules) {
      if (isTTY) {
        try {
          ...
          const confirmation = yield promptBool(msg, {
            trailing: '\n'
          })
    ...
    if (aliasDepl) {
      aliasedDeployment = yield this.findDeployment(aliasDepl.deploymentId)
      if (
        aliasedDeployment &&
    ...
    alias = yield this.maybeSetUpDomain(alias, domains, currentTeam, user)

    const aliasTime = Date.now()
    const newAlias = yield this.createAlias(depl, alias)
    if (!newAlias) {
      throw new Error(
    ...
    if (aliasedDeployment && shouldScaleDown) {
      ...
      yield this.setScale(aliasedDeployment.uid, { min: 0, max: 1 })
  ...
  createAlias(depl, alias) {
    return this.retry((bail, attempt) => __async(function*(){
      ...
      const res = yield this._fetch(`/now/deployments/${depl.uid}/aliases`, {
        method: 'POST',
        body: { alias }
      })
  ...
  setupRecord(domain, name) {return __async(function*(){
    yield this.setupDomain(domain)
    ...
    return this.retry((bail, attempt) => __async(function*(){
      ...
      const res = yield this._fetch(`/domains/${domain}/records`, {
        method: 'POST',
  ...
  maybeSetUpDomain(alias, domains, currentTeam, user) {return __async(function*(){
    const gracefulExit = () => {
      this.close()
      domains.close()
    ...
    if (!/\.now\.sh$/.test(alias)) {
      ...
      const canBePurchased = yield domains.status(parsed.domain)
      ...
      if (canBePurchased) {
        ...
        const confirmation = yield promptBool(`Buy now for ${chalk.bold(`$${price}`)} (${chalk.bold((currentTeam && currentTeam.slug) || user.username || user.email)})?`)
        eraseLines(1)
        if (!confirmation) {
          info('Aborted')
          gracefulExit()
        ...
        let domain
        try {
          domain = yield domains.buy(parsed.domain)
        } catch (err) {
          stopSpinner()
          treatBuyError(err)
          gracefulExit()
        ...
        try {
          domainInfo = yield this.setupDomain(parsed.domain)
        } catch (err) {
          if (this._debug) {
      ...
      if (!usingZeitWorld && domainInfo) {
        if (domainInfo.verified) {
          ...
          const { verified, created } = yield this.setupDomain(domain, {
            isExternal: true
          })
          if (!(created && verified)) {
            const e = new Error(
      ...
      try {
        if (!skipDNSVerification) {
          yield this.verifyOwnership(alias)
        }
      } catch (err) {
        if (err.userError) {
          ...
          try {
            if (usingZeitWorld) {
              ...
              if (_record === '') {
                yield this.setupRecord(_domain, '*')
              }

              yield this.setupRecord(_domain, _record)

              this.recordSetup = true
              console.log('> DNS Configured! Verifying propagation…')

              try {
                yield this.retry(() => this.verifyOwnership(alias), {
                  retries: 10,
                  maxTimeout: 8000
                })
              } catch (err2) {
      ...
      if (!usingZeitWorld && !skipDNSVerification) {
        ...
        const { uid, verified, created } = yield this.setupDomain(domain, {
          isExternal: true
        })
        if (!(created && verified)) {
          const e = new Error(
  ...
  verifyOwnership(domain) {
    return this.retry(
      bail => __async(function*(){
        const targets = yield resolve4('alias.zeit.co')
