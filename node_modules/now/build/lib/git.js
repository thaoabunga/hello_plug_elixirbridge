...
const renameRepoDir = (pathParts, tmpDir) => __async(function*(){
  const tmpContents = yield fs.readdir(tmpDir.path)

  const oldTemp = path.join(tmpDir.path, tmpContents[0])
  const newTemp = path.join(tmpDir.path, pathParts.main.replace('/', '-'))

  yield fs.rename(oldTemp, newTemp)
  tmpDir.path = newTemp
...
const downloadRepo = repoPath => __async(function*(){
  ...
  try {
    yield cloneRepo(pathParts, tmpDir)
  } catch (err) {
    gitInstalled = false
  }

  if (gitInstalled) {
    const renaming = yield renameRepoDir(pathParts, tmpDir)
    return renaming
  }
  ...
  try {
    yield download(url, tmpDir.path, {
      extract: true
    })
  } catch (err) {
    tmpDir.cleanup()
...
const fromGit = (path, debug) => __async(function*(){
  let tmpDir = false

  try {
    tmpDir = yield downloadRepo(path)
  } catch (err) {
    if (debug) {
