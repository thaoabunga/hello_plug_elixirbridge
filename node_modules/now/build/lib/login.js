...
function register(url, { retryEmail = false } = {}) {return __async(function*(){
  ...
  let user
  try {
    user = (yield (yield fetch(`${url}/www/user`, {
      headers: {
...
module.exports = function(url) {return __async(function*(){
  ...
  yield cfg.merge(loginData)
  yield cfg.remove('currentTeam') // Make sure to logout the team too
  yield cfg.remove('email') // Remove old schema from previus versions
