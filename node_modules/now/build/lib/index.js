...
module.exports = class Now extends EventEmitter {
  ...
  create(
    ...
    if (type === 'npm') {
      files = yield getNpmFiles(path, pkg, nowConfig, opts)
      ...
      files = yield getDockerFiles(path, nowConfig, opts)
    }
    ...
    if (type === 'npm' && forwardNpm) {
      authToken =
        ((yield readAuthToken(path))) || ((yield readAuthToken(homedir())))
    }
    ...
    const hashes = yield hash(files, isStatic, pkgDetails)
    ...
    const deployment = yield this.retry(bail => __async(function*(){
      if (this._debug) {
      ...
      const files = yield Promise.all(
        Array.prototype.concat.apply(
          [],
          yield Promise.all(
            Array.from(this._files).map(([sha, { data, names }]) => __async(function*(){
              const statFn = followSymlinks ? stat : lstat

              return names.map(name => __async(function*(){
                let mode

                const getMode = () => __async(function*(){
                  const st = yield statFn(name)
                  return st.mode
                }())
      ...
      const res = yield this._fetch('/now/create', {
        method: 'POST',
        body: {
          env,
        ...
        parts.shift().map(sha =>
          retry(
            (bail, attempt) => __async(function*(){
              ...
              const res = yield this._fetch('/now/sync', {
                method: 'POST',
                headers: {
              ...
              if (
                ...
                return bail(yield responseError(res))
              }
  ...
  listSecrets() {return __async(function*(){
  ...
  list(app) {return __async(function*(){
    const query = app ? `?app=${encodeURIComponent(app)}` : ''

    const { deployments } = yield this.retry(
      bail => __async(function*(){
        if (this._debug) {
          console.time('> [debug] /list')
        }

        const res = yield this._fetch('/now/list' + query)
  ...
  listInstances(deploymentId) {return __async(function*(){
    const { instances } = yield this.retry(
      bail => __async(function*(){
        if (this._debug) {
  ...
  logs(deploymentIdOrURL, { types, limit, query, since, until } = {}) {return __async(function*(){
  ...
  listAliases(deploymentId) {return __async(function*(){
    return this.retry(() => __async(function*(){
      const res = yield this._fetch(
        deploymentId
  ...
  last(app) {return __async(function*(){
    const deployments = yield this.list(app)
  ...
  listDomains() {return __async(function*(){
    return this.retry((bail, attempt) => __async(function*(){
      if (this._debug) {
        console.time(`> [debug] #${attempt} GET /domains`)
      }

      const res = yield this._fetch('/domains')
  ...
  getDomain(domain) {return __async(function*(){
    return this.retry((bail, attempt) => __async(function*(){
      if (this._debug) {
        console.time(`> [debug] #${attempt} GET /domains/${domain}`)
      }

      const res = yield this._fetch(`/domains/${domain}`)
  ...
  createCert(domain, { renew } = {}) {
    return this.retry(
      (bail, attempt) => __async(function*(){
        ...
        const res = yield this._fetch('/now/certs', {
          method: 'POST',
          body: {
            domains: [domain],
            renew
  ...
  deleteCert(domain) {
    return this.retry((bail, attempt) => __async(function*(){
      ...
      const res = yield this._fetch(`/now/certs/${domain}`, {
        method: 'DELETE'
      })
  ...
  remove(deploymentId, { hard }) {return __async(function*(){
    const data = { deploymentId, hard }

    yield this.retry(bail => __async(function*(){
      if (this._debug) {
        console.time('> [debug] /remove')
      }

      const res = yield this._fetch('/now/remove', {
        method: 'DELETE',
        body: data
      })
  ...
  setScale(nameOrId, scale) {
    return this.retry((bail, attempt) => __async(function*(){
      ...
      const res = yield this._fetch(`/now/deployments/${nameOrId}/instances`, {
        method: 'POST',
        body: scale
      })
  ...
  unfreeze(depl) {return __async(function*(){
    return this.retry(bail => __async(function*(){
      const res = yield fetch(`https://${depl.url}`)
  ...
  getPlanMax() {return __async(function*(){
    return 10
  }())}
}
...
function responseError(res) {return __async(function*(){
  let message
  let userError
...
function readAuthToken(path, name = '.npmrc') {return __async(function*(){
  try {
    const contents = yield readFile(resolvePath(path, name), 'utf8')
